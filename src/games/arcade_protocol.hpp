//
// arcade_protocol.hpp for  in /home/hazer/cpp_arcade/snake_SDL/include
//
// Made by hazer
// Login   <hazer@epitech.net>
//
// Started on  Mon Mar  6 11:46:15 2017 hazer
// Last update Mon Mar  6 12:01:09 2017 hazer
//
...
namespace	arcade
{
  enum class	CommandType : uint16_t
  {
    WHERE_AM_I = 0,	// RETURN A WHERE AM I STRUCTURE
      ...
      ILLEGAL	= 8,	// THE INSTRUCTION WAS I LL EG AL
      PLAY = 9	// PLAY A ROUND
      };
  enum class	TileType : uint16_t
  {
    EMPTY = 0,		// TILE WHERE THE CHARACTER CAN GO
      ...
      MY_SHOOT = 5,	// YOUROWN SHOT
      POWERUP = 6,	// POWERUP
      OTHER= 7		// ANYTHING THAT WILL BE IGNORED BY THE KOALINETTE
      };

  // The format is width, height, and width * height * sizeof(TileType)quantityofTileType
  struct		GetMap
  {
    CommandType	type ;
    uint16_t	width ;
    uint16_t	height;
    TileType	tile[0];
    //SPECIFY AN ADDITIONALSIZE WHILE ALLOCATING
  ...
  struct		WhereAmI
  {
    CommandType		type;
    uint16_t		lenght;
    Position		position[0];	//  SPECIFY  AN  ADDITIONAL  S I Z E  WHILE  ALLOCATING
  }	__attribute__((packed));
}

#endif
// __ARCADE_PROTOCOL_HPP__
